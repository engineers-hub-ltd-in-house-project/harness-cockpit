    ...
    <html lang={lang} data-theme="dark">
      <head>
        ...
        <style>{`
          ...
          .lang-toggle button { padding: 4px 12px; font-size: 0.8rem; cursor: pointer; }
        `}</style>
      </head>
      <body>
        <main class="container" style="max-width: 720px; padding-top: 2rem; position: relative;">
          <div class="lang-toggle">
            <button
              hx-post={`/api/lang?lang=${otherLang}`}
...
export const Badge: FC<{ status: string }> = ({ status }) => (
  <span class={`badge badge-${status}`}>{t(`status.${status}`)}</span>
);
