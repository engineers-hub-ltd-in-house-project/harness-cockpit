...
app.post("/", async (c) => {
  ...
  updateStep("generate", { status: "running", output: "" });

  const result = await exec(
    ["bash", scriptPath("generate-config.sh"), state.projectId, state.targetProject],
    { cwd: state.targetProject }
  );
  const output = result.stdout + (result.stderr ? "\n" + result.stderr : "");
  const ok = result.exitCode === 0;

  updateStep("generate", { status: ok ? "done" : "error", output });
