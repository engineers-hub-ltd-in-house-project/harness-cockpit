...
P1からP4に至る各段階での運用作業のリファレンス。本書では日次・週次の確認項目、トークンローテーション、コスト監視、およびPhase移行判断基準を定める。
...
P1（完全パーミッシブ）の目標は2週間分の行動ベースラインデータの蓄積である。日次・週次で以下のサイクルを回す。
...
このデータはP2でのルール設計の基礎データとなる。使用頻度の高いツールから優先的にルールを検討する。
...
P2では、蓄積データを基に初期ルールセット（5-10個）をパーミッシブモードで定義し、`WOULD_BLOCK` イベントの蓄積を開始する。
